...
class HeartRateMonitor:
    ...
    def bandpass_filter(self, data, lowcut, highcut, fs, order=3):
        ...
        y = scipy_signal.filtfilt(b, a, data, padlen=min(len(data)-1, 3*max(len(a), len(b))))
        return y
    ...
    def calculate_hr_peaks(self, signal, fs):
        ...
        valid_intervals = intervals[(intervals > 0.33) & (intervals < 1.5)]  # 40-180 BPM
    ...
    def calculate_vitals(self):
        ...
        min_samples = int(self.fps * 8)  # Need at least 8 seconds
        ...
        try:
            ...
            if len(self.time_buffer) > 1:
                ...
                actual_fs = len(signal) / actual_duration if actual_duration > 0 else self.fps
            else:
                actual_fs = self.fps
            ...
            hr_peaks = self.calculate_hr_peaks(signal_filtered, actual_fs)
            
            # Combine results
            hr_estimates = [hr for hr in [hr_fft, hr_peaks] if hr is not None and 45 < hr < 160]
            ...
            # Calculate simple HRV estimate (standard deviation of recent HR values)
            hrv = np.std(list(self.hr_history)) if len(self.hr_history) > 2 else 0
            ...
            if len(faces) > 0:
                ...
                if self.last_face is not None:
                    ...
                    face = min(faces, key=lambda f: 
                        abs(f[0] - self.last_face[0]) + abs(f[1] - self.last_face[1]))
                else:
                ...
                if self.current_hr is not None:
                    ...
                    cv2.putText(frame, f"Heart Rate: {self.current_hr:.0f} BPM", 
                    ...
                    cv2.putText(frame, "Status: MEASURING", 
                    ...
                    cv2.putText(frame, f"Calibrating: {progress:.0f}%", 
                               (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 
                               0.8, (0, 165, 255), 2)
                    cv2.putText(frame, "Please stay still...", 
                               (20, 80), cv2.FONT_HERSHEY_SIMPLEX, 
                               0.6, (200, 200, 200), 2)
            else:
                self.last_face = None
                cv2.putText(frame, "No face detected", 
                           (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 
                           0.8, (0, 0, 255), 2)
                cv2.putText(frame, "Please face the camera", 
                           (20, 80), cv2.FONT_HERSHEY_SIMPLEX, 
                           0.6, (200, 200, 200), 2)
            
            # Add tips at bottom
            cv2.putText(frame, "TIP: Stay still, good lighting helps!", 
