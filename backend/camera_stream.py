...
class WebSocketHeartRateMonitor(HeartRateMonitor):
    ...
    def get_frame_data(self):
        ...
        if face_detected:
            ...
            if self.current_hr is not None:
                ...
                cv2.putText(frame, f"Heart Rate: {self.current_hr:.0f} BPM", 
                           (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 1, hr_color, 2)
        ...
        cv2.putText(frame, "TIP: Stay still, good lighting helps!", 
                   (20, frame.shape[0] - 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (150, 150, 150), 1)
        ...
        return {
            ...
            'hrv': round(self.current_hrv, 1) if self.current_hrv else None,
            'confidence': min(100, len(self.hr_history) * 10),
            'calibration_progress': min(100, len(self.signal_buffer) / (self.fps * 8) * 100) if self.current_hr is None else 100
        }
